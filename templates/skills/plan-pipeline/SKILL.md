...
When the user asks generically to plan phase NN, verify if research exists (`docs/decisions/technical-decisions-*.md`). If NO decisions doc exists, direct them to `/research phase NN` FIRST! `/research phase NN` MUST ALWAYS run before `/plan-context NN` in phase mode because `/plan-context` requires the decisions doc as its primary input. If a decisions doc already exists (or in task mode), direct them to `/plan-context <arg>`.
